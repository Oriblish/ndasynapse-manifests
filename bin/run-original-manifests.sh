...
manifest_type=$(sed -n ${LINE}p /config/manifest_types_unversioned.txt)
...
cat /config/collection_ids.txt | xargs -P8 -I{} -n 1 sh -c 'query-nda --config /root/ndaconfig.json get-collection-manifests --collection_id ${2} --manifest_type ${1} > /tmp/nda-manifest-${1}-${2}-ORIGINAL.csv' -- ${manifest_type} {}
