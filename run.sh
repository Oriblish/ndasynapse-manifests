...
for manifest_type in genomics_subject02 genomics_sample03 nichd_btb02 ; do
    manifest_guid_data.py --config /root/ndaconfig.json --manifest_type ${manifest_type} >| > /tmp/nda-manifests-${manifest_type}-LIVE.csv && synapse store --parentId syn11452082 /tmp/nda-manifests-${manifest_type}-LIVE.csv;
done
